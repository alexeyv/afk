...
@pytest.fixture
def turn_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> tuple[Path, Driver]:
...
class TestTurnStateMachine:
    def test_turn_starts_in_initial_state(self, turn_env: tuple[Path, Driver]) -> None:
        """New Turn starts in INITIAL state."""
        root_dir, driver = turn_env
        turn = Turn(driver, root_dir)
        assert turn.state == TurnState.INITIAL

    def test_turn_allocates_number_on_start(
        self, turn_env: tuple[Path, Driver]
    ) -> None:
        """Turn number is allocated when Turn is started (not created)."""
        root_dir, driver = turn_env
        turn1 = Turn(driver, root_dir)
        turn2 = Turn(driver, root_dir)
    ...
    def test_start_creates_log_file(self, turn_env: tuple[Path, Driver]) -> None:
    ...
    def test_start_captures_head_before(self, turn_env: tuple[Path, Driver]) -> None:
    ...
    def test_cannot_start_twice(self, turn_env: tuple[Path, Driver]) -> None:
    ...
    def test_execute_requires_in_progress(self, turn_env: tuple[Path, Driver]) -> None:
    ...
    def test_execute_returns_exit_code(self, turn_env: tuple[Path, Driver]) -> None:
    ...
    def test_finish_returns_turn_result(self, turn_env: tuple[Path, Driver]) -> None:
    ...
    def test_finish_logs_end_marker(self, turn_env: tuple[Path, Driver]) -> None:
    ...
    def test_finish_requires_in_progress(self, turn_env: tuple[Path, Driver]) -> None:
    ...
    def test_abort_transitions_to_aborted(self, turn_env: tuple[Path, Driver]) -> None:
    ...
    def test_abort_logs_abort_marker(self, turn_env: tuple[Path, Driver]) -> None:
    ...
    def test_abort_reraises_exception(self, turn_env: tuple[Path, Driver]) -> None:
    ...
    def test_abort_requires_in_progress(self, turn_env: tuple[Path, Driver]) -> None:
...
class TestTurnValidation:
    def test_rejects_non_driver(self, turn_env: tuple[Path, Driver]) -> None:
        """Turn requires Driver instance."""
        root_dir, _ = turn_env
        with pytest.raises(TypeError, match="expected Driver"):
            Turn("not a driver", root_dir)  # type: ignore[arg-type]

    def test_rejects_non_path_session_root(self, turn_env: tuple[Path, Driver]) -> None:
        """Turn requires Path for session_root."""
        _, driver = turn_env
        with pytest.raises(TypeError, match="expected Path"):
            Turn(driver, "/tmp/path")  # type: ignore[arg-type]

    def test_rejects_relative_session_root(self, turn_env: tuple[Path, Driver]) -> None:
        """Turn requires absolute path for session_root."""
        _, driver = turn_env
        with pytest.raises(ValueError, match="absolute"):
            Turn(driver, Path("relative/path"))
    ...
    def test_log_file_raises_before_start(self, turn_env: tuple[Path, Driver]) -> None:
...
class TestTurnNumberAllocation:
    def test_sequential_allocation(self, turn_env: tuple[Path, Driver]) -> None:
        """Turn numbers are allocated sequentially on start()."""
        root_dir, driver = turn_env
        turns = [Turn(driver, root_dir) for _ in range(5)]
    ...
    def test_resume_from_specific_number(self, turn_env: tuple[Path, Driver]) -> None:
    ...
    def test_resume_from_rejects_zero(self, turn_env: tuple[Path, Driver]) -> None:
    ...
    def test_resume_from_rejects_max(self, turn_env: tuple[Path, Driver]) -> None:
...
class TestTurnRepr:
    def test_repr_shows_number_and_state(self, turn_env: tuple[Path, Driver]) -> None:
